...
export interface Config {
  ...
  pk?: string;
  /** `fsk_…` project API key for the response and management tools. */
  apiKey?: string;
  /** Origin the API key belongs to (guards against cross-deployment reuse). */
  apiKeyApi?: string;
...
/**
 * The `fsk_…` project API key, or undefined. Env wins; a config key is only used
 * when its bound origin matches, exactly as the login token is treated — a key
 * minted for fillo.so must never be sent to a different deployment because a
 * checked-in MCP client config pointed `FILLO_API` somewhere else. The key is
 * the fallback credential for every management tool, so it needs the same guard
 * the more obvious credential already has.
 */
export function resolveApiKey(): string | undefined {
  ...
  // Nothing writes this key today — it is hand-placed — so a config that records
  // no origin is accepted for the default deployment only, like an older token.
  if (!boundTo) return apiOrigin() === DEFAULT_API ? cfg.apiKey : undefined;
  return boundTo === apiOrigin() ? cfg.apiKey : undefined;
}
