...
import { ALL_TOOLS, OUTWARD_TOOLS } from "./manifest.mjs";
...
test("concurrent tool requests carry their own registered names without arguments", async () => {
  const mock = await startMock(() => ({ status: 200, json: { forms: [], workspace: {}, project: {} } }));
  const server = await startServer(mock.origin, { FILLO_TOKEN: "fcli_test" });
  track(server, mock);
  await Promise.all([server.callTool("fillo_list_forms"), server.callTool("fillo_whoami")]);
  assert.equal(mock.requests.length, 2);
  for (const request of mock.requests) {
    assert.equal(request.client, `@usefillo/mcp@${PKG_VERSION}`);
    assert.equal(request.tool, request.url.endsWith("/forms") ? "fillo_list_forms" : "fillo_whoami");
  }
});

test("lists exactly the tools the manifest names", async () => {
  const mock = await startMock(() => ({ status: 404, json: {} }));
  const server = await startServer(mock.origin);
  track(server, mock);
  const tools = await server.listTools();
  // A tool name is a public contract (saved agent plans, prompts, the hosted
  // server), so adding or renaming one is an edit to test/manifest.mjs.
  assert.equal(new Set(TOOL_NAMES).size, TOOL_NAMES.length, "manifest has a duplicate name");
...
test("every tool carries honest annotations (reads and writes distinguished)", async () => {
  ...
  // openWorldHint marks exactly the calls whose effect leaves the workspace:
  // push (publishes by default), explicit publish, and every Tier B action.
  // A client shows a stronger warning for these, so the set must not drift.
  const outward = new Set([...OUTWARD_TOOLS, "fillo_push_form", "fillo_publish_form"]);
  for (const tool of tools) {
    assert.equal(tool.annotations.openWorldHint, outward.has(tool.name), tool.name);
  }
  // A read can never be an outward action — that combination would mean a
  // "safe to repeat" tool that reaches a third party.
  for (const tool of tools) {
    if (tool.annotations.openWorldHint) {
      assert.equal(tool.annotations.readOnlyHint, false, `${tool.name} is outward AND read-only`);
    }
  }
...
test("provision: forwards an optional promptCopyId to the API", async () => {
  const mock = await startMock(({ url }) =>
    url === "/api/v1/workspaces/provision"
      ? {
          status: 201,
  ...
  const res = await server.callTool("fillo_provision_workspace", {
    email: "dev@fillo.dev",
    promptCopyId: "2f1c8a0e-4b3d-4e9a-9c1b-7a6d5e4c3b2a",
  });
...
const LIBRARY_FORM = {
  id: "sample-task-ease",
  title: "Task ease",
  useWhen: "Immediately after the task.",
  avoidWhen: "Before someone attempts the task.",
  sourceNote: "An editorial adaptation, not an official company form.",
  sources: [
    {
      title: "Source",
  ...
  setup: [
    {
      id: "task",
      label: "Task",
      hint: "Use the task just completed.",
      example: "Export a report",
      maxLength: 100,
    },
  ],
  beforePublish: ["Name the task and select the eligible cohort."],
  interpretation: "Keep cohort and timing consistent.",
  measurement: {
    name: "Example measure",
    version: "1",
    itemIds: ["ease"],
    scoring: "Read separately.",
    preserve: "Keep wording and endpoints.",
  },
  schema: {
    version: 1,
    title: "Task ease",
    pages: [{ id: "main", blocks: [{ id: "ease", kind: "rating", label: "How easy?", max: 5 }] }],
  },
};

function libraryCatalogFixture(forms, extra = {}) {
  return {
    version: 1,
    status: "published",
    updated: "2026-09-11",
    instructions: "/library.md",
    previewCollectsResponses: false,
    publishing:
      "Use the existing authenticated Fillo setup and publishing flow. This catalog does not publish forms.",
    categories: ["All forms", "Feedback"],
    total: forms.length,
    count: forms.length,
    offset: 0,
    nextOffset: null,
...
test("library tools discover bounded summaries and retrieve exact schema with context without credentials", async () => {
  const { schema, ...summary } = LIBRARY_FORM;
  const mock = await startMock(({ method, url }) => {
    assert.equal(method, "GET");
    const parsed = new URL(url, "http://mock.test");
    assert.equal(parsed.pathname, "/library.json");
    if (parsed.searchParams.has("id")) {
      assert.equal(parsed.searchParams.get("id"), LIBRARY_FORM.id);
      return { status: 200, json: libraryCatalogFixture([LIBRARY_FORM]) };
    }
    assert.equal(parsed.searchParams.get("q"), "task & ease");
    assert.equal(parsed.searchParams.get("category"), "Feedback");
    assert.equal(parsed.searchParams.get("detail"), "summary");
    assert.equal(parsed.searchParams.get("limit"), "2");
    assert.equal(parsed.searchParams.get("offset"), "4");
    return {
      status: 200,
      json: libraryCatalogFixture([summary], { total: 6, offset: 4, nextOffset: 5 }),
    };
  });
  const server = await startServer(mock.origin, { FILLO_API_KEY: "fsk_should_not_be_sent" });
  track(server, mock);
  const found = await server.callTool("fillo_search_library", {
    q: "task & ease",
    category: "Feedback",
    limit: 2,
    offset: 4,
  ...
  assert.deepEqual(found.data.forms, [summary]);
  assert.equal(found.data.nextOffset, 5);
  const fetched = await server.rpc("tools/call", {
    name: "fillo_get_library_form",
    arguments: { id: found.data.forms[0].id },
  });
  assert.equal(fetched.result.isError, undefined);
  assert.deepEqual(fetched.result.structuredContent.forms, [LIBRARY_FORM]);
  assert.ok(mock.requests.every((req) => req.auth === undefined));
  assert.ok(mock.requests.every((req) => req.client === `@usefillo/mcp@${PKG_VERSION}`));
});

test("library tools validate inputs before HTTP and return actionable unknown-id errors", async () => {
  const mock = await startMock(() => ({ status: 404, json: { error: "Form not found" } }));
  const server = await startServer(mock.origin);
  track(server, mock);
  for (const [name, args] of [
    ["fillo_search_library", { q: "x".repeat(201) }],
    ["fillo_search_library", { limit: 13 }],
    ["fillo_search_library", { offset: -1 }],
    ["fillo_search_library", { category: "unknown" }],
    ["fillo_search_library", { url: "http://localhost/private" }],
    ["fillo_get_library_form", { id: "https://example.test/private" }],
    ["fillo_get_library_form", { id: "../private" }],
    ["fillo_get_library_form", { id: "" }],
    ["fillo_get_library_form", { id: LIBRARY_FORM.id, publish: true }],
  ]) {
    const res = await server.callTool(name, args);
    assert.ok(res.isError || res.error, JSON.stringify(args));
  }
  assert.equal(mock.requests.length, 0);
  const missing = await server.callTool("fillo_get_library_form", { id: "no-such-form" });
  assert.equal(missing.isError, true);
  assert.match(missing.text, /fillo_search_library/);
});

test("library search handles empty results and rejects malformed or oversized result pages", async () => {
  const mock = await startMock(({ url }) => {
    const q = new URL(url, "http://mock.test").searchParams.get("q");
    return {
      status: 200,
      json:
        q === "empty"
          ? libraryCatalogFixture([])
          : q === "malformed"
            ? {}
            : libraryCatalogFixture([LIBRARY_FORM, LIBRARY_FORM]),
    };
  });
  const server = await startServer(mock.origin);
  track(server, mock);
  const empty = await server.callTool("fillo_search_library", { q: "empty" });
  assert.equal(empty.isError, false);
  assert.equal(empty.data.count, 0);
  for (const q of ["malformed", "oversized"]) {
    const res = await server.callTool("fillo_search_library", { q, limit: 1 });
    assert.equal(res.isError, true);
    assert.match(res.text, /unexpected catalog response/);
  }
});
