...
- @usefillo/core@0.23.1

## 0.23.0

### Patch Changes

- @usefillo/core@0.23.0

## 0.22.0

### Patch Changes

- 292105d: Keep arrow-key scrolling working in WebKit when a form's page headings are visually hidden. Pagination now focuses the visible question group when the heading has no visible layout size.
- Updated dependencies [cc0659c]
  - @usefillo/core@0.22.0

## 0.21.0

### Minor Changes

- 3c4ae08: Add opt-in page browsing for render-only form previews. Native Next keeps validation and branching, while Skip opens later pages without answering or sending data. Back returns through the pages visited, and preview submissions keep the native success screen without following live redirects.

### Patch Changes

- 3c4ae08: Use straight SVG selection checks instead of font-dependent checkmark characters. Consolidate default form typography into five text sizes and three weights, with matching sizes for questions, answer controls, and helper text. Both renderers preserve host font inheritance and custom styling.
- 983d97f: Keep icon choice labels aligned when selected by placing the checkmark after the label. Applies to single- and multiple-choice fields.
- Updated dependencies [3c4ae08]
...
- b373032: Mark automatically generated "Powered by Fillo" attribution links as `nofollow` so site-wide embeds do not pass ranking credit.
...
- 84a3cb8: React renderer accessibility and correctness fixes:
  - File-upload field: the accessible name, required/invalid state, and error association move to the operable dropzone (they were on the hidden `<input>`, so screen readers heard an unnamed button and focus-to-error was a no-op).
  - Focus is moved and announced on view changes: the success screen is focused/`role="status"`, and multi-page Next/Back moves focus to the new page.
  - Phone field no longer causes a hydration mismatch (initial country is seeded deterministically, then upgraded from timezone/locale after mount), and its country picker returns focus to the trigger on Escape/close.
  - The component-override maps (`FieldComponents`/`CustomComponents`) now narrow the field to its concrete kind, so overrides read `field.options`/`field.config` without a cast.
  - Custom field components re-render on `api.status`/`api.uploading`/`api.pageIndex` changes instead of reading stale engine state.
  - Inline callback props (`onError`/`onChange`/`onSubmitted`) are ref-stabilized so they can't retrigger the load effect and remount the form.
