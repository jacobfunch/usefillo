...
const browseForm = {
  ...
  settings: { successTitle: "Thank you", successMessage: "Preview complete" },
  pages: [
    {
      id: "judgment",
      blocks: [{ id: "answer", kind: "short_text", label: "Was it helpful?", required: true }],
      next: [{ when: [{ fieldId: "answer", op: "eq", value: "yes" }], to: "end" }],
    },
    {
      id: "diagnosis",
      title: "Tell us more",
      blocks: [
        { id: "reason", kind: "short_text", label: "What went wrong?", required: true },
        {
          id: "detail",
          kind: "short_text",
          label: "Which fact?",
          visibleIf: [{ fieldId: "reason", op: "eq", value: "incorrect" }],
        },
      ],
    },
    {
      id: "finish",
      title: "Anything else?",
      blocks: [{ id: "note", kind: "long_text", label: "Note" }],
    },
  ],
};

async function nativeSubmit(target) {
  await act(async () => {
    target
      .querySelector("form")
...
test("pagination focuses visible content instead of clipped or hidden headings", async () => {
  ...
  try {
    for (const [width, height] of [
      ...
      dom.window.HTMLElement.prototype.getBoundingClientRect = function () {
        if (this.matches(".fillo-page-title")) {
      ...
      try {
        await act(async () => target.querySelector("[data-fillo-preview-skip]").click());
        assert.equal(target.querySelector("form").dataset.page, "2");
        assert.equal(
          document.activeElement,
          target.querySelector(width > 1 ? ".fillo-page-title" : ".fillo-blocks"),
          `${width}×${height} heading: focus must land on visible page content`,
        );
      } finally {
...
test("render-only previews keep native controls and conditionals, with explicit transportless Skip", async () => {
  const calls = [];
  const pages = [];
  let completion;
  ...
  const { target, root } = await mount(
    React.createElement(FilloForm, {
      form: { ...browseForm, settings: { ...browseForm.settings, saveProgress: true } },
      formId: "should-never-send",
      client,
      renderOnly: true,
      previewNavigation: true,
      onPageChange: (index) => pages.push(index),
      onSubmitted: (...args) => {
        completion = args;
      },
    }),
  ...
  const buttons = () =>
    Array.from(target.querySelectorAll(".fillo-footer button"), (b) => b.textContent);
  assert.deepEqual(buttons(), ["Skip", "Next"]);
  await nativeSubmit(target);
  assert.equal(target.querySelector("form").dataset.page, "1", "Next validates normally");
  assert.ok(target.querySelector('[data-field="answer"] [data-fillo="error"]'));
  await type(target.querySelector('[data-field="answer"] input'), "yes");
  assert.equal(target.querySelector('[role="progressbar"]').getAttribute("aria-valuemax"), "3");
  assert.deepEqual(
    buttons(),
    ["Skip", "Submit"],
    "native early-end routing changes the primary action",
  );
  await act(async () => target.querySelector("[data-fillo-preview-skip]").click());
  assert.equal(target.querySelector("form").dataset.page, "2");
  assert.equal(target.querySelector('[role="progressbar"]').getAttribute("aria-valuenow"), "2");
  assert.deepEqual(buttons(), ["Back", "Skip", "Next"]);
  assert.equal(target.querySelector('[data-field="detail"]'), null);
  assert.equal(
    target.querySelector("details"),
    null,
    "conditional fields aren't exposed as preview accordions",
  );
  await type(target.querySelector('[data-field="reason"] input'), "incorrect");
  assert.ok(
    target.querySelector('[data-field="detail"]'),
    "conditionals use normal field visibility",
  );
  await nativeSubmit(target);
  assert.equal(target.querySelector("form").dataset.page, "3");
  assert.equal(target.querySelector('[role="progressbar"]').getAttribute("aria-valuenow"), "3");
  assert.deepEqual(buttons(), ["Back", "Submit"], "there is no Skip on the physical final page");
  await act(async () => target.querySelector(".fillo-footer button").click());
  assert.equal(target.querySelector("form").dataset.page, "2");
  await act(async () => target.querySelector(".fillo-footer button").click());
  assert.equal(
    target.querySelector("form").dataset.page,
    "1",
    "Back restores the original branch source",
  );
  await nativeSubmit(target);
  assert.ok(target.querySelector('[data-fillo="success"]'));
  assert.match(target.textContent, /Thank you/);
  assert.equal(completion[0], undefined, "local success has no response id");
  assert.deepEqual(completion[1], { answer: "yes" }, "off-route answers aren't submitted");
  assert.deepEqual(pages, [1, 2, 1, 0]);
  assert.deepEqual(calls, [], "render-only navigation, inputs and submit perform no transport");
  await act(async () => root.unmount());
});

test("live forms ignore forged previewNavigation and retain validation", async () => {
  const { target, root } = await mount(
    React.createElement(FilloForm, {
      form: browseForm,
      formId: "live",
  ...
  assert.equal(target.querySelector("[data-fillo-preview-skip]"), null);
  await nativeSubmit(target);
  assert.equal(target.querySelector("form").dataset.page, "1");
  assert.ok(target.querySelector('[data-field="answer"] [data-fillo="error"]'));
  await act(async () => root.unmount());
});

test("branched preview progress matches server HTML during hydration", async () => {
    ...
    pages: [
      {
        ...browseForm.pages[0],
        next: [
          { when: [{ fieldId: "answer", op: "eq", value: "no" }], to: "diagnosis" },
          { when: [], to: "end" },
        ],
      },
      browseForm.pages[1],
  ...
  for (const previewNavigation of [false, true]) {
    const element = React.createElement(FilloForm, {
      form: branchingForm,
      renderOnly: true,
      previewNavigation,
      devNotices: false,
    ...
    try {
      ...
      const before = target.querySelector('[role="progressbar"]').outerHTML;
      assert.match(before, new RegExp(`aria-valuemax="${previewNavigation ? 2 : 1}"`));
      assert.match(before, new RegExp(`--fillo-progress-value:${previewNavigation ? 50 : 100}%`));
      ...
      assert.deepEqual(errors, [], "initial hydration must agree with the server render");
      assert.equal(target.querySelector('[role="progressbar"]').outerHTML, before);
      assert.equal(Boolean(target.querySelector("[data-fillo-preview-skip]")), previewNavigation);
    } finally {
      globalThis.window = previousWindow;
...
test("one-page previews have only native Submit and respect localized success copy", async () => {
  const { target, root } = await mount(
    React.createElement(FilloForm, {
      form: {
        ...form,
        pages: [{ id: "only", blocks: [{ id: "note", kind: "short_text", label: "Note" }] }],
      },
      renderOnly: true,
      previewNavigation: true,
      strings: { previewSkip: "Spring over", submit: "Send", successTitle: "Tak" },
    }),
  );
  assert.deepEqual(
    Array.from(target.querySelectorAll(".fillo-footer button"), (b) => b.textContent),
  ...
  assert.equal(target.querySelector("select"), null);
  await nativeSubmit(target);
  assert.equal(target.querySelector(".fillo-success-title").textContent, "Tak");
  await act(async () => root.unmount());
});

test("auto-submit previews retain a primary action on inspected off-route pages", async () => {
  const { target, root } = await mount(
    React.createElement(FilloForm, {
      form: {
        version: 1,
        ...
        pages: [
          {
            ...
            next: [{ when: [{ fieldId: "helpful", op: "eq", value: "yes" }], to: "end" }],
          },
          { id: "notes", blocks: [{ id: "note", kind: "long_text", label: "Note" }] },
        ],
      },
      initialData: { helpful: "yes" },
      renderOnly: true,
      previewNavigation: true,
    }),
  );
  await act(async () => target.querySelector("[data-fillo-preview-skip]").click());
  assert.equal(target.querySelector("form").dataset.page, "2");
  assert.equal(target.querySelector('button[type="submit"]').textContent, "Submit");
  await nativeSubmit(target);
  assert.ok(target.querySelector('[data-fillo="success"]'));
  await act(async () => root.unmount());
});
...
test("generated Powered by Fillo attribution is nofollow", async () => {
  const { target } = await mount(
    React.createElement(FilloForm, { form, formId: "f-attribution", client: fakeClient() }),
  );
  const badge = target.querySelector(".fillo-powered");
  assert.ok(badge);
  assert.equal(badge.relList.contains("nofollow"), true);
  assert.equal(badge.relList.contains("noopener"), true);
  assert.equal(badge.relList.contains("noreferrer"), true);
});
...
for (const kind of ["select", "multi_select"]) {
  test(`${kind} icon choices keep their label before the trailing selection marker`, async () => {
    const iconForm = {
      version: 1,
      title: "Feedback",
      settings: {},
      pages: [{
        ...
        blocks: [{
          id: "helpful",
          kind,
          label: "Did this help you?",
          options: [
            { id: "yes", label: "Yes, it helped", icon: "thumbs_up" },
            { id: "no", label: "No, not quite", icon: "thumbs_down" },
          ],
        }],
      }],
    };
    const { target, root } = await mount(React.createElement(FilloForm, {
      form: iconForm,
      formId: "f-icon-choice",
      client: fakeClient(),
    }));

    function assertOption(id, selected) {
      const row = target.querySelector(`[data-option="${id}"]`);
      const icon = row.querySelector(".fillo-option-icon");
      const label = row.querySelector('[data-fillo="optionLabel"]');
      const check = row.querySelector(".fillo-option-check");
      assert.equal(row.querySelector("input").checked, selected);
      // The marker's auto margin must not push the text away from its icon.
      assert.ok(icon.nextElementSibling === label, "label stays next to the icon");
      assert.equal(label.textContent, id === "yes" ? "Yes, it helped" : "No, not quite");
      if (selected) {
        assert.ok(check, "selected icon option has a non-color indicator");
        assert.equal(check.tagName.toLowerCase(), "svg");
        assert.equal(check.textContent, "");
        assert.equal(check.querySelector("path").getAttribute("d"), "M4 12l5 5L20 6");
        assert.equal(check.getAttribute("focusable"), "false");
        assert.ok(label.nextElementSibling === check, "check follows the label");
        assert.ok(row.lastElementChild === check, "check is the trailing element");
        assert.equal(check.getAttribute("aria-hidden"), "true");
      } else {
        assert.equal(check, null, "unselected option has no check");
      }
    }

    try {
      assertOption("yes", false);
      assertOption("no", false);
      await act(async () => target.querySelector('[data-option="yes"]').click());
      assertOption("yes", true);
      assertOption("no", false);
      await act(async () => target.querySelector('[data-option="no"]').click());
      assertOption("yes", kind === "multi_select");
      assertOption("no", true);
