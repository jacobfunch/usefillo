...
test("a 200-file field uploads every selected file and stops at its configured limit", async () => {
  const uploaded = [];
  const client = fakeClient({
    uploadFile: async (_formId, file) => {
      uploaded.push(file.name);
      return { fileId: file.name, name: file.name, size: file.size, mime: "text/plain" };
    },
  });
  const { target, root } = await mount(
    React.createElement(FilloForm, { form: uploadForm(200), formId: "f-batch", client }),
  );
  const input = target.querySelector('input[type="file"]');
  ...
  await flush();
  assert.equal(uploaded.length, 200);
