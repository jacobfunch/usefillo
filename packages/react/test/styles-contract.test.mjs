...
// Parity above makes a second pass over the identical DOM stylesheet redundant.
const css = readFileSync(REACT_CSS_URL, "utf8");
for (const spec of TARGET_SIZE_SELECTORS) {
  const dims = [spec.checkWidth && "min-width", spec.checkHeight && "min-height"]
    .filter(Boolean)
    .join(" and ");
  test(`target size: ${spec.selector} declares ${dims} >= ${spec.minRem}rem`, () => {
    const decls = declarationsFor(css, spec.selector);
    assert.ok(
      decls.length > 0,
      `no rule block found for selector ${spec.selector} — was it renamed?`,
    );
    if (spec.checkWidth) {
      const w = minRemValue(decls, "min-width");
      assert.ok(
        w !== null && w >= spec.minRem,
        `${spec.selector} must declare min-width >= ${spec.minRem}rem (found ${w === null ? "none" : `${w}rem`})`,
      );
    }
    if (spec.checkHeight) {
      ...
      assert.ok(
        ...
        `${spec.selector} must declare min-height >= ${spec.minRem}rem (found ${h === null ? "none" : `${h}rem`})`,
      );
    }
  });
}

test("validation styling uses the error token and supports accessibility preferences", () => {
  // Preserve error-token and accessibility support, not grid placement,
  // decorative borders, or the exact focus-ring width and opacity.
  assert.match(
    css,
  ...
  assert.match(
    css,
    /:where\(\.fillo-field--error\) \.fillo-input:focus-visible\s*\{[^}]*box-shadow[^;]*var\(--fillo-error\)[^}]*outline\s*:\s*none/s,
  );
  assert.match(
    css,
    /:where\(\.fillo-field--error\) \.fillo-dropzone:focus-visible\s*\{[^}]*box-shadow[^;]*var\(--fillo-error\)[^}]*outline\s*:\s*none/s,
  );
  assert.match(
    css,
    /:where\(\.fillo-field--error\) \.fillo-number:focus-within\s*\{[^}]*box-shadow[^;]*var\(--fillo-error\)/s,
  );
  assert.match(
    css,
  ...
  assert.match(
    css,
    /:where\(\.fillo-field--error\) \.fillo-option:focus-within\s*\{[^}]*box-shadow[^;]*var\(--fillo-error\)/s,
  );
  assert.match(
    css,
    /:where\(\.fillo-field--error\) :where\(\.fillo-toggle-input:focus-visible\) \+ \.fillo-toggle-track\s*\{[^}]*var\(--fillo-error\)/s,
  );
  assert.match(
    css,
    /:where\(\.fillo-field--error\) \.fillo-scale-step:focus-visible\s*\{[^}]*box-shadow[^;]*var\(--fillo-error\)/s,
  );
  assert.match(
    css,
