...
function ResolvedForm(
  ...
  // Announce page changes, including preview Skip. Failed validation leaves
  // the index unchanged so focusFirstInvalid still handles that path.
  ...
  useEffect(() => {
    ...
    // Prefer a named heading; fall back to the blocks when titles are hidden.
    ...
    // A visually hidden heading can trap WebKit's arrow-key scrolling after
    // focus. Use the visible blocks when the host clips or hides its headings.
    const headingRect = heading?.getBoundingClientRect();
    const target =
      headingRect && headingRect.width > 1 && headingRect.height > 1 ? heading : blocksRef.current;
    if (target) {
  ...
  // Challenges only gate Submit, including auto-submit forms. Keep its explicit
  // button disabled without a token; the server independently enforces the check.
  ...
  // Browsable previews count every physical page. Real-form progress uses the
  // REACHABLE page sequence (the same walk render/validate/
  ...
  const progressSeq = previewNavigation
    ? schema.pages.map((page) => page.id)
    : reachablePageSequence(api.form, api.data);
        ...
        >
          ...
          {showPrimaryButton && (
            <button
              ...
              data-fillo="button"
              disabled={
                api.status === "submitting" || api.uploading || (showChallenge && !challengeToken)
              }
            >
              {api.uploading
                ? strings.uploading
