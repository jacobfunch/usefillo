...
void (
  <FilloForm form={schema} renderOnly previewNavigation onPageChange={(index) => index.toFixed()} />
);

// @ts-expect-error Preview skipping cannot be enabled on a live targeted form.
void (<FilloForm form={schema} formId="published-form" client={client} previewNavigation />);
// @ts-expect-error Skip requires explicit renderOnly, not the legacy preview flag.
void (<FilloForm form={schema} skipValidation previewNavigation />);
