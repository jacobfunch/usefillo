import { clientTelemetryHeaders } from "../lib/client-telemetry.js";
...
const BOOTSTRAP_USAGE =
  "To set up a workspace I need the human's real work email for the recovery link. " +
  "Ask the human for their work email once — never invent an address or use a documentation placeholder — then re-run: " +
  'npx @usefillo/cli@latest agent bootstrap --email <work-email> [--name "Your Name"]. ' +
...
function agentHelp() {
  ...
  ${boldRaw("Commands")}
    agent bootstrap     Set up a workspace and install the Build with Fillo skill
                       ${dimRaw("--email <work-email>   provision a fresh workspace (ask the human first; a TTY confirms git identity)")}
                       ${dimRaw('--name "Your Name"        optional display name')}
                       ${dimRaw("--workspace-name <name>   name the new workspace (default: this repo/directory)")}
                       ${dimRaw("--pc <uuid>               optional marketing stitch id from /agents?pc=")}
