...
async function status(handle: string | undefined, flags: Flags) {
  ...
  // The status body IS the form, bare, exactly as the `fsk_` twin returns it.
  // `--json` still prints it under `form`: that is this CLI's own contract.
  const body = (await readJson(res)) as {
    id?: string;
    name?: string;
  ...
  if (!res.ok || !body.id) die(body.error ?? `status failed (${res.status}).`);
  if (jsonMode(flags)) return emitResult({ form: body });
  const form = body as typeof body & {
    id: string;
    name: string;
    status: "draft" | "published";
    url: string;
