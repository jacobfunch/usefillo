...
// The dashboard patch keys, grouped by how a `key=value` argument parses. The
// nine operational keys plus the six presentation keys the builder edits in its
// schema panel — mirrored from lib/form-settings-contract.ts on the server,
// which is what actually validates them.
...
type SettingsBody = { settings: Record<string, unknown> };
...
async function get(handle: string | undefined, flags: Flags) {
  ...
  const body = await callApi<SettingsBody>(
    `/cli/forms/${encodeURIComponent(handle)}/settings`,
    {},
    { fallback: failed("settings get"), expect: (b) => Boolean(b.settings) },
  );
...
async function set(handle: string | undefined, pairs: string[], flags: Flags) {
  ...
  const body = await callApi<SettingsBody>(
    ...
    {
      fallback: failed("settings set"),
      expect: (b) => Boolean(b.settings),
      on: (res, b) => {
        if (res.status === 400) {
          die(`${b.error ?? "Invalid settings patch"} — keys sent: ${sentKeys.join(", ")}.`);
        }
        // Every form `fillo push` creates is code-managed, and the server
        // rejects saveProgress/responseLimit/trust on those (they live in the
        // synced schema). Pass the server's exact reason through, then name the
        // fix — the same next step the --help caveat gives.
        if (res.status === 409 && /controlled by the synced schema/i.test(b.error ?? "")) {
          die(`${b.error} Set them in the form schema you \`fillo push\`, then push again.`);
        }
      },
    },
  );
...
function settingsHelp() {
  ...
  ${boldRaw("Presentation")} ${dimRaw("(what the respondent sees; schema-level, see the caveat below)")}
    ${dimRaw("submitMode=button|auto, submitLabel, successTitle, successMessage")}
    ${dimRaw("redirectUrl (http/https), showProgress=true|false")}

  ${boldRaw("Controlled by your form schema")} ${dimRaw("(set them there and re-push, not here)")}
    ${dimRaw("saveProgress, responseLimit, trust, and every presentation key above")}
