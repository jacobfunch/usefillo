import { api, assertMintedSecret, readJson, requireToken } from "../lib/api.js";
...
/** Irreversible scopes all end in `:delete` (forms/workspace/responses/respondents). */
...
async function createKey(flags: Flags) {
  ...
  if (!preset && (!scopes || scopes.length === 0)) {
    die(
      "Choose the key's access: --preset read|agent|full, or --scopes with explicit scopes " +
        "(the only way to grant forms:delete, responses:delete, respondents:delete, or workspace:delete).",
    );
  }
  ...
  assertMintedSecret(body.key, "fsk_", "key");
...
function keysHelp() {
  ...
  ${boldRaw("Commands")}
    keys create --name <name>   Mint a key — the plaintext is shown once, store it
                       ...
                       ${dimRaw("                           forms:delete, responses:delete, respondents:delete, workspace:delete")}
