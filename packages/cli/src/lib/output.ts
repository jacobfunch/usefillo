...
/**
 * The date half of an ISO timestamp — what a table column or a one-line status
 * shows. Tolerates "" (the caller's `?? ""` for a missing field) by returning
 * "", so a blank cell stays blank instead of reading as a bogus date.
 */
export const dateOnly = (iso: string) => iso.slice(0, 10);

/** "1 response" / "2 responses" — the -s pluralization, spelled once. */
