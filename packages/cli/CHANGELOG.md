...
- 6b66be8: Everything the dashboard can do now works from the terminal.
  - **Form lifecycle:** `fillo pull` writes a form back out in the `fillo push`
    format; `unpublish`, `discard`, `duplicate`, `rename`, `versions`,
    `storage set`, and `storage folder` cover the rest. `fillo settings set` also
    accepts the presentation keys (`submitMode`, `submitLabel`, `successTitle`,
    `successMessage`, `redirectUrl`, `showProgress`) on dashboard-built forms.
  - **Destinations:** `fillo sheets`, `fillo notion`, and `fillo hubspot` turn a
    form's Google Sheets, Notion, or HubSpot destination on and off;
    `fillo slack enable|disable|status` does the same for Slack;
    `fillo connections` lists the workspace's provider accounts, switches which
    one a project uses, and removes one; `fillo discord rename` and
    `disconnect-server` complete Discord; `fillo notion connect` and
    `fillo hubspot connect` print an OAuth link and wait, like `fillo slack
connect`. `fillo webhooks add|set` takes `--auth none|bearer|x-api-key`; the
    secret comes from `FILLO_WEBHOOK_AUTH_SECRET` or a hidden prompt, never an
    argument.
  - **Responses and delivery:** `fillo responses list --held`, `responses
release` (loops until the held queue is empty), `responses delete`,
    `respondents list|delete`, `deliveries status|retry|redeliver`, `drafts`,
    and `insights`. `deliveries status` names a destination by connector or by
    host plus a short fingerprint rather than printing the target URL.
  - **Workspace:** `fillo workspace rename`, `project rename`, `members
role|remove`, `tokens list|revoke`, `sync-tokens create|list|revoke`,
    `developers policy|origins|identity`, and `mcp list|revoke`.

  One confirmation rule across all of it. Routine commands just run. Outward
  commands (publish, unpublish, release, enabling a destination, adding a
  webhook, invites, role changes, origins, code-sync policy, identity
  verification) print what will happen and, whenever an agent or a pipe is
  driving rather than a person at a terminal, refuse without a bare `--confirm`.
  Irreversible commands (deletes, removals, revocations, disabling identity
  verification) need the target typed back with `--confirm "<target>"`; `--yes`
  never substitutes. Minted secrets are shown once and never listed. The bundled
  Build with Fillo skill teaches agents to ask the person they work for before
  passing `--confirm`.
...
- 173a894: `fillo discord` goes bot-first: `connect` runs the one-consent server install (Fillo's bot + first channel), `enable --channel <channelId>` aims a form at any channel of a connected server (the server mints or reuses the webhook), and `status` lists connected servers with their ids. Re-running `enable` on an already-live form without `--channel` no longer silently re-points it (which also used to clear the Early signal window).

### Patch Changes

- 8d9bbd7: `agent bootstrap` without `--email` now tells the coding agent to ask the human for their real work email and re-run with `--email <their email>`, instead of teaching `you@company.com` as a copy-pasteable address. The bundled Build-with-Fillo skill requires the same ask-once step before first-time setup.
- 8d9bbd7: `fillo agent bootstrap` now tells agents to ask the human for a work email instead of modeling `you@company.com`, while keeping existing-account login guidance.
