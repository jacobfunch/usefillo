...
try {
  ...
  assert.match(
    storageAmbiguous.stdout,
    /Uploads for storage=null forms resolve to: no durable storage yet/,
  );
  ...
  // AUTH names the mode Fillo uses to authenticate TO the receiver; "—" is none.
  assert.match(whList.stdout, /ID +URL +EVENTS +AUTH +CREATED/);
  assert.match(
    whList.stdout,
    /wh1 +https:\/\/a\.test\/hook +response\.created +— +2026-07-20 10:00/,
  );
  ...
  // Adding a webhook is Tier B: from then on every response leaves Fillo for a
  // host Fillo does not control, so agent mode refuses without a bare --confirm
  // and the notice names the receiving host, not the full URL.
  resetState();
  const whAddNoConfirm = await runCli(["webhooks", "add", "f1", "--url", "https://b.test/hook"]);
  assert.notEqual(whAddNoConfirm.code, 0);
  assert.match(whAddNoConfirm.stderr, /posted to b\.test/);
  assert.equal(requests.length, 0, "a refused add must never reach the API");
  ...
  resetState({
    settingsPatchError: {
      ...
      body: {
        error:
          "Save progress, response limits, the trust policy, and the submit/redirect presentation are controlled by the synced schema.",
      },
    },
  });
  ...
  assert.match(
    setCodeManaged.stderr,
    /Set them in the form schema you `fillo push`, then push again/,
  );
  ...
  // FILLO_TTY=1 stands in for the terminal, because `fillo delete` shares
  // lib/confirm.ts's rule that a caller with no TTY is told to pass --confirm
  // rather than prompted (test-confirm.mjs forces the lane the same way).
  resetState();
  const delInteractive = await runCli(
    ["delete", "form", "f1"],
    { FILLO_TTY: "1" },
    "Contact Form\n",
  );
  ...
  resetState();
  const delMismatchLocal = await runCli(
    ["delete", "form", "f1"],
    { FILLO_TTY: "1" },
    "Wrong Title\n",
  );
  ...
  // No TTY and no --confirm: a pipeline is told to pass one rather than handed a
  // prompt that would read whatever happens to be on stdin.
  resetState();
  const delPiped = await runCli(["delete", "form", "f1"], {}, "Contact Form\n");
  assert.notEqual(delPiped.code, 0);
  assert.match(delPiped.stderr, /Refusing to delete without confirmation/);
  assert.equal(requests.length, 0);
  ...
  resetState({ formMissing: true });
  const delMissingInteractive = await runCli(["delete", "form", "ghost"], { FILLO_TTY: "1" }, "");
