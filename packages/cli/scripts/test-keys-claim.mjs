...
try {
  ...
  // ---------- keys --help ----------
  // Keep the available access choices discoverable without pinning terminal layout.
  const keysHelp = await runCli(["keys"]);
  assert.equal(keysHelp.code, 0, keysHelp.stderr);
  assert.match(keysHelp.stdout, /--preset\s+read\|agent\|full/);
  assert.match(keysHelp.stdout, /--scopes\s+a,b,c/);
  for (const scope of ["forms:delete", "responses:delete", "workspace:delete"]) {
    assert.ok(keysHelp.stdout.includes(scope), `help must list the explicit scope ${scope}`);
  }
  noAnsi(keysHelp);
