...
try {
  ...
  assert.match(
    ...
    /Ask the human for their work email once/,
    "guides the agent to ask once, not invent an address",
  );
  assert.match(bareBootstrap.stderr, /--email <work-email>/);
  assert.doesNotMatch(bareBootstrap.stderr, /you@company\.com/);
  ...
  // ---------- agent bootstrap --pc: forwards the marketing stitch id ----------
  reset();
  const stitched = await runCli(
    [
      "agent",
      "bootstrap",
      "--email",
      "boot@x.dev",
      "--pc",
      "2f1c8a0e-4b3d-4e9a-9c1b-7a6d5e4c3b2a",
      "--json",
    ],
    {},
  );
  assert.equal(stitched.code, 0, stitched.stderr);
  assert.deepEqual(JSON.parse(provisionRequests()[0].body), {
    email: "boot@x.dev",
    source: "cli",
    workspaceName: derivedWorkspace,
