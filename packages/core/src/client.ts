...
// Bound memory and provider requests when someone selects a large batch.
...
export class FilloClient {
  ...
  private uploadQueue = new Set<() => void>();
  ...
  async uploadFile(
    ...
    await this.acquireUploadSlot(options.signal);
    try {
      options.signal?.throwIfAborted();
      return await this.performUpload(formId, file, options);
    } finally {
      this.activeUploads -= 1;
      const next = this.uploadQueue.values().next().value;
      if (next) {
        this.uploadQueue.delete(next);
        next();
      }
    }
  }

  private async acquireUploadSlot(signal?: AbortSignal): Promise<void> {
    signal?.throwIfAborted();
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      const aborted = () => {
        this.uploadQueue.delete(start);
        reject(signal?.reason);
      };
      this.uploadQueue.add(start);
      signal?.addEventListener("abort", aborted, { once: true });
    });
  }
