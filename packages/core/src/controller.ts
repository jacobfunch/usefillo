...
export interface FormControllerOptions {
  ...
  /**
   * Enable explicit page skipping in a transportless preview. Requires
   * skipValidation and no client, formId, or resolver. Native next() still
   * validates; skipPreviewPage() alone bypasses validation and page routing.
   */
...
export interface FormController {
  ...
  next(): void;
  /** Browse the next physical page in an explicitly enabled local preview. */
...
export function createFormController(options: FormControllerOptions): FormController {
  ...
  let pageIndex = 0;
  let previewStartPageId: string | undefined;
  const previewHistory: { pageIndex: number; startPageId?: string }[] = [];

  function previewNavigationEnabled(): boolean {
    return (
      options.previewNavigation === true &&
      options.skipValidation === true &&
      !client &&
      !formId &&
      !options.resolveFormId
    );
  }

  function navigationSequence(): string[] {
    return reachablePageSequence(
      form,
      data,
      previewNavigationEnabled() ? previewStartPageId : undefined,
    );
  }

  function rememberPreviewPage(): void {
    if (previewNavigationEnabled()) {
      previewHistory.push({ pageIndex, startPageId: previewStartPageId });
    }
  }
  ...
  function computeSnapshot(): FormControllerState {
    ...
    return {
      ...
      isLastPage:
        previewNavigationEnabled() && previewStartPageId !== undefined
          ? navigationSequence().at(-1) === page.id
          : isTerminalPage(form, page.id, data),
  ...
  function validatePage(index: number): Record<string, string> {
    ...
    for (const block of visiblePageBlocks(form, target, data)) {
      ...
      // A local preview cannot upload, so its disabled picker cannot satisfy
      // required-file validation. Other fields retain normal Next validation.
      if (previewNavigationEnabled() && block.kind === "file_upload") continue;
  ...
  function next() {
    const pageCount = form.pages.length;
    const clamped = Math.min(pageIndex, Math.max(pageCount - 1, 0));
    if (!options.skipValidation || options.previewNavigation === true) {
    ...
    const terminal =
      (previewNavigationEnabled() && previewStartPageId !== undefined
        ? seq.at(-1) === current?.id
        : current
          ? isTerminalPage(form, current.id, data)
          : true) ||
      pos < 0 ||
      pos + 1 >= seq.length;
    if (terminal) {
  ...
  function back() {
    errors = {};
    const previous = previewNavigationEnabled() ? previewHistory.pop() : undefined;
    if (previous) {
      pageIndex = Math.min(previous.pageIndex, form.pages.length - 1);
      previewStartPageId = previous.startPageId;
      submitError = undefined;
      notify();
      return;
    }
  ...
  function skipPreviewPage() {
    if (
      !previewNavigationEnabled() ||
      status === "submitting" ||
      status === "submitted" ||
    ...
    rememberPreviewPage();
    pageIndex += 1;
    // A skipped-to page may be outside the answer's normal route. Walk that
    // branch with the same core logic, retaining all fields for conditions.
    previewStartPageId = form.pages[pageIndex]?.id;
    errors = {};
    submitError = undefined;
    notify();
  }
  ...
  async function submit() {
    ...
    if (previewNavigationEnabled()) {
      // A skipped-to page can be outside validateResponse's normal route.
      // Its visible controls still behave like a form when Submit is pressed.
      errors = validatePage(pageIndex);
