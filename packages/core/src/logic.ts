 ...
 */
export function reachablePageSequence(
  form: FormSchema,
  data: ResponseData,
  /** Optional starting page for inspecting a branch in a local form preview. */
  startPageId?: string,
): string[] {
  const seq: string[] = [];
  if (form.pages.length === 0) return seq;
  const seen = new Set<string>();
  let index = startPageId === undefined ? 0 : form.pages.findIndex((p) => p.id === startPageId);
