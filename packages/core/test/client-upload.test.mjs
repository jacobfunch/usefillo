...
function completedUpload(id) {
  return Response.json({
    id, status: "complete",
    file: { fileId: id, name: "upload.bin", size: 0, mime: "application/octet-stream" },
  });
}

test("200 simultaneous upload calls finish with at most four active transfers", async () => {
  let active = 0;
  let peak = 0;
  let starts = 0;
  const client = createClient({
    fetch: async (url) => {
      ...
      if (target.pathname.endsWith("/forms/f1/uploads")) {
        const id = `file-${starts++}`;
        active += 1;
        peak = Math.max(peak, active);
        return Response.json({
          id, status: "pending",
          transport: { type: "s3-put", uploadUrl: `https://storage.test/${id}` },
        });
      }
      await new Promise((resolve) => setImmediate(resolve));
      if (target.hostname === "storage.test") return new Response(null, { status: 200 });
      if (target.pathname.endsWith("/complete")) {
        active -= 1;
        return completedUpload(target.pathname.split("/").at(-2));
      }
      throw new Error(`Unexpected upload request: ${url}`);
    },
  });
  const files = await Promise.all(Array.from({ length: 200 }, () =>
    client.uploadFile("f1", new Blob([]), { fieldId: "files" }),
  ));
  assert.equal(peak, 4);
  assert.equal(new Set(files.map((file) => file.fileId)).size, 200);
});

test("queued uploads cancel without opening a session and failed uploads release their slot", async () => {
  const pending = [];
  let starts = 0;
  const client = createClient({
    fetch: async () => {
  ...
  const upload = (signal) => client.uploadFile("f1", new Blob([]), { fieldId: "files", signal });
  const active = Array.from({ length: 4 }, () => upload());
  const failed = assert.rejects(active[0], (error) => error.status === 400);
  const cancelled = new AbortController();
  const cancelledResult = assert.rejects(upload(cancelled.signal), { name: "AbortError" });
  const next = upload();
  await new Promise((resolve) => setImmediate(resolve));
  assert.equal(starts, 4);
  cancelled.abort();
  await cancelledResult;
  pending.shift()(Response.json({ error: "Invalid file" }, { status: 400 }));
  await failed;
  await new Promise((resolve) => setImmediate(resolve));
  assert.equal(starts, 5, "cancelled queue entry must never start");
  pending.forEach((resolve, i) => resolve(completedUpload(`file-${i}`)));
  await Promise.all([...active.slice(1), next]);
  const alreadyCancelled = new AbortController();
  alreadyCancelled.abort();
  await assert.rejects(upload(alreadyCancelled.signal), { name: "AbortError" });
