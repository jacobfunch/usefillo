...
test("explicit preview Skip bypasses unanswered pages while Next still validates", async () => {
  let completed = 0;
  const c = createFormController({
    form: twoPage,
    skipValidation: true,
  ...
  c.skipPreviewPage();
  assert.equal(c.getState().pageIndex, 1);
  assert.deepEqual(c.getState().errors, {});
  c.skipPreviewPage();
  assert.equal(c.getState().pageIndex, 1, "Skip at the physical end is a no-op");
  assert.equal(c.getState().status, "idle");
  assert.equal(completed, 0, "browsing never submits");
  await c.submit();
  assert.equal(c.getState().pageIndex, 0, "native Submit still asks for missing required answers");
...
test("preview Skip can inspect a branch after early end and Back returns through visited pages", () => {
  const c = createFormController({
    form: jumpForm,
    skipValidation: true,
    previewNavigation: true,
    initialData: { route: "stop" },
  });
  assert.equal(c.getState().isLastPage, true);
  c.skipPreviewPage();
  assert.equal(c.getState().pageIndex, 1, "the otherwise unreachable page is inspectable");
  assert.equal(c.getState().isLastPage, false);
  c.next();
  assert.equal(c.getState().pageIndex, 1, "native Next validates the inspected branch");
  c.setValue("detail", "The answer was unclear");
  c.next();
  assert.equal(c.getState().pageIndex, 2, "native Next follows the inspected page's route");
  assert.equal(c.getState().isLastPage, true);
  c.back();
  assert.equal(c.getState().pageIndex, 1);
  c.back();
  assert.equal(c.getState().pageIndex, 0);
  assert.equal(c.getState().isLastPage, true, "original early-end branch is restored");
  c.setValue("route", "skip");
  c.next();
  assert.equal(c.getState().pageIndex, 2, "Next still honors the original forward jump");
  c.back();
  assert.equal(c.getState().pageIndex, 0, "Back crosses the same jump");
});

test("preview Skip is disabled when any live transport context exists or resolves later", async () => {
  const calls = [];
  const client = {
    startSession: async () => null,
    reportProgress() {},
  ...
  for (const context of [
    ...
    assert.equal(c.getState().pageIndex, 0);
    c.next();
    assert.equal(c.getState().pageIndex, 0, "Next validates even when preview access is refused");
    c.destroy();
  }
  const late = createFormController({
    form: twoPage,
    skipValidation: true,
    previewNavigation: true,
  });
  late.setContext({ client, formId: "live" });
  late.skipPreviewPage();
  late.next();
  assert.equal(late.getState().pageIndex, 0);
  await late.submit();
  assert.equal(late.getState().status, "idle");
  assert.deepEqual(calls, []);
});

test("Submit validates the current inspected terminal page even when the normal branch ends earlier", async () => {
  const c = createFormController({
    form: { ...jumpForm, pages: jumpForm.pages.slice(0, 2) },
    initialData: { route: "stop" },
    skipValidation: true,
    previewNavigation: true,
  });
  ...
  await c.submit();
  assert.equal(c.getState().status, "idle");
  assert.equal(c.getState().pageIndex, 1, "the invalid inspected page stays in view");
  assert.ok(c.getState().errors.detail);
  c.setValue("detail", "The answer was unclear");
  await c.submit();
  assert.equal(c.getState().status, "submitted");
});
