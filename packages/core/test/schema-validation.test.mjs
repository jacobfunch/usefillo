...
import { normalizeFormSchema, normalizeFormTheme, validateFormSchema, validateField } from "../dist/index.js";
...
test("normalizes renderer-sensitive field data", () => {
    ...
    pages: [
      {
        ...
        blocks: [
          ...
          { id: "files", kind: "file_upload", label: "Files", maxFiles: 999, maxFileSizeMb: 9999 },
        ],
      },
    ],
    settings: {
  ...
  assert.equal(result.schema.pages[0].blocks[3].maxFiles, 200);
...
test("upload fields preserve limits through 200 and enforce the configured answer count", () => {
  const files = Array.from({ length: 201 }, (_, i) => ({
    fileId: `file-${i}`, name: `file-${i}.txt`, size: 1, mime: "text/plain",
  }));
  for (const [input, expected] of [[undefined, 1], [0, 1], [5, 5], [20, 20], [21, 21], [200, 200], [201, 200]]) {
    const result = normalizeFormSchema({
      version: 1, title: "Files", settings: {},
      pages: [{ id: "p1", blocks: [{ id: "files", kind: "file_upload", label: "Files", maxFiles: input }] }],
    });
    assert.equal(result.ok, true);
    const field = result.schema.pages[0].blocks[0];
    assert.equal(field.maxFiles, expected);
    assert.equal(validateField(field, files.slice(0, expected)), null);
    assert.match(validateField(field, files.slice(0, expected + 1)), /At most/);
  }
});
