...
test("DOM-generated Powered by Fillo attribution is nofollow", () => {
  const target = mount(oneField);
...
test("development keeps transient mount failure fillable and resolves again at submit", { timeout: 5_000 }, async () => {
  const prevEnv = process.env.NODE_ENV;
  const prevWarn = console.warn;
  ...
  try {
    let syncCalls = 0;
    const submits = [];
    let reportMountFailure;
    const mountFailure = new Promise((resolve) => { reportMountFailure = resolve; });
    let reportSubmitted;
    const submitted = new Promise((resolve) => { reportSubmitted = resolve; });
    const client = {
    ...
    const target = mount(
      ...
      { client, onError: reportMountFailure, onSubmitted: reportSubmitted },
    );
    // Wait for the bounded mount retries to finish before testing a new submit.
    // A fixed timer can overlap the two sync operations on a busy runner.
    assert.equal((await mountFailure).status, 503);
...
test("a 200-file selection is retained by the DOM renderer and a 201st file is refused", async () => {
  const target = mount({
    version: 1, title: "Files", settings: {},
    pages: [{ id: "p1", blocks: [{ id: "files", kind: "file_upload", label: "Files", maxFiles: 200 }] }],
  });
  const input = target.querySelector('input[type="file"]');
  Object.defineProperty(input, "files", {
    configurable: true,
    value: Array.from({ length: 201 }, (_, i) => new File(["a"], `${i}.txt`)),
  });
  dispatch(input, "change");
  await tick();
  await tick();
  assert.equal(target.querySelectorAll(".fillo-file--done").length, 200);
  assert.equal(target.querySelector(".fillo-dropzone"), null);
});
