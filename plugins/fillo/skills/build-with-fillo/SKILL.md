...
- Ask the human before you pass `--confirm`. The CLI runs routine commands
  straight away, but it stops on two kinds of change and refuses them in agent
  mode without that flag. Outward changes — publishing or unpublishing,
  releasing held responses, re-sending responses, turning on an integration that
  sends answers to a third party, changing a member's role, changing the
  code-sync policy or allowed origins, enabling identity verification — take a
  bare `--confirm`. Irreversible ones — deleting a form, response, or
  respondent, removing a member, disconnecting an integration account or Discord
  server, revoking an API key, connector token, sync token, or MCP grant — take
  the target typed out: `--confirm "<exact email, id, or name>"`. A bare
  `--confirm` never satisfies a typed one and `--yes` never substitutes for
  either. Put the choice to the person you are working for in one line, wait for
  their answer, and only then re-run with the flag. Never add `--confirm`
  speculatively, and never add it to get past a refusal. Over MCP the same
  actions return a one-time approval URL instead: print it, let a workspace
  manager click it, and do not retry the call meanwhile. See
  [references/operations.md](references/operations.md).
